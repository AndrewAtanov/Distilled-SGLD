...
parser.add_argument('--nruns', default=int(10), type=int)
...
teacher_lr = lambda t: decay_lr(t, 1e-5, 0.5, 20000)
...
for random_state in args.states:
    ...
    # noise target
    y[np.random.choice(list(range(400)), replace=False, size=5)] *= 10
    np.save('y_noised_{}.npy'.format(random_state), y)
    np.save('X_{}.npy'.format(random_state), X)
    ...
    for n_run in range(args.nruns):
        ...
        for t in range(T):
            ...
            if (t+1) % burn_iter == 0:
                ...
                t_train_rmse = np.sqrt(np.mean((t_train_mu - y)**2))
                t_test_rmse = np.sqrt(np.mean((average_test_predict - y_test)**2))
