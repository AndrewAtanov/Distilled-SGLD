...
from mnist_nets import mnist_fc
...
teacher_lr = tf.placeholder(tf.float32)
optimizer = LangevinSampler(teacher_lr, teacher_lr)
...
teacher_lr_policy = lambda t: decay_lr(t, args.lr, 0.5, 1000)
...
for k, [batch_xs, batch_ys] in enumerate(batch_iterator(x_train, y_train,
    ...
    _, ce_summ, acc_summ = session.run([teacher_train_op, cross_entropy_summary,
        ...
        input_y: batch_ys,
        mode: 'Train',
        teacher_lr: teacher_lr_policy(k + 1)
    })
